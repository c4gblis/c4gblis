...
function get_new_test($specimen_id, &$candidate_test_list)
{
...
function add_specimens_random($num_specimens, $user_list=array())
{
	...
	while($count < $num_specimens)
	{
		...
		$specimen->dateCollected = get_random_date($ts_start, $ts_end);
		...
		do
		{
			$test_entry = get_new_test($specimen_id_count, $candidate_test_list);
			add_test($test_entry);
