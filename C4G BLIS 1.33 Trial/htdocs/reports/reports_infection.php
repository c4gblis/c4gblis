...
{ 
	$('#showtablelink').text("View Trends");
	$('#trendsDiv').hide();
	/*
	$('#prevalance').toggle();
	$('#stat_table').toggle();      
	var linktext = $('#showtablelink').text();
	if(linktext.indexOf("Trends") != -1) {
		$('#stat_graph_bar').hide();
		$('#showgraphlink').hide();
		$('#showtablelink').text("Back");
		//view_prevalance();
		//$('#prevalance_graph').show();
		viewTrends();
	}
	else {
		$('#showtablelink').text("Trends");
		$('#showgraphlink').text("Graphs");
		$('#showgraphlink').show();
	}
	*/
}
...
<?php /* |<a href="javascript:toggle_stat_graph();" id='showgraphlink'>Show Graph</a> */ ?>
 | <a href="javascript:toggle_stat_table();" id='showtablelink'>Hide Trends</a>
 | <a href='reports.php?show_p'>&laquo; <?php echo LangUtil::$pageTerms['MSG_BACKTOREPORTS']; ?></a>
<br><br>
<?php
